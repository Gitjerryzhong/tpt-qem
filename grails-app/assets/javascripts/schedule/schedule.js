//= require teach/model/arrangement.js
...
$.widget("ui.schedule", {
	...
	_create: function () {
		...
		// create model
		this.week = this.options.currentWeek
		$.each(this.options.arrangements, function() {
			that.arrangements.push(new Arrangement(this, that.options.forms, that.options.freeArrangements))
		});
    ...
    renderCell: function(arr) {
		...
		if(text) {
			text += "<hr>";
			if(!cell(row, col).data('item').free && !arr.free)
				cell(row, col).addClass("conflict");
		}
		text += arr.courseText() + "<br>" +
    ...
    reset: function() {	    	
    	$("td.arr", this.$el)
    		.prop("rowspan", 1)
            .removeClass("enabled rc conflict")
    	    .removeData("item")
    	    .show()
            .empty();
    }
