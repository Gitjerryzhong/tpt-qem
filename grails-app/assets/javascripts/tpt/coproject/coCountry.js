coProjectApp.controller('CoCountryCtrl',['$rootScope','$scope','$http',function($rootScope,$scope,$http){ //国外大学管理
	$scope.coTypeId=0;
   $scope.coType={}
   $scope.coTypes ={}
    $scope.saveCoType = function(){ //保存    	
    	$http({
			method:'POST',
			url:"/tms/tptCoPrjAdmin/saveCoCountry",
    ...
    $scope.showCoType = function(){ //显示类型
    	$http({
			 method:'GET',
				url:"/tms/tptCoPrjAdmin/showCoCountry"
		 }).success(function(data) {
			 if(data!=null && data.coTypes!=null){
				 $scope.coTypes=data.coTypes;					 
			 }else {				
    ...
    $scope.rmCoType = function(id){
    	$http({
    		method:'GET',
			url:"/tms/tptCoPrjAdmin/rmCoCountry",
			params:{
				coTypeId:id
			}
    	}).success(function(data) {
