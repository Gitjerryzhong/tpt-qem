...
<table class="table table-bordered table-striped" id="updateInfo">
			...
			<tbody id="listBody">
				<tr ng-if="updateTypes[2].selected">
					<td>项目名称</td>
					<td>{{taskView.projectName}}</td>
					<td><span ng-class="{'Emphasis': commitAction && !updateView.projectName }">项目名称</span></td>
					<td><input type="text" name="projectName"  class="form-control" placeholder="您的项目名称*" ng-model="updateView.projectName" required/></td>
				</tr>
				<tr ng-if="updateTypes[6].selected">
					<td>参与人</td>
					<td>{{taskView.memberstr}}</td>
					<td>参与人</td>
				...
				<tr ng-if="updateTypes[1].selected">
					<td>中期预期</td>
					<td>{{taskView.expectedMid}}</td>
					<td><span ng-class="{'Emphasis':myForm.expectedMid.$error.pattern || (commitAction && !updateView.expectedMid) }">中期预期</span></td>
					<td><input type="text"  class="form-control" name="expectedMid"  ng-pattern="/^20\d{2}$/" placeholder="请输入4位年份" ng-model="updateView.expectedMid" required/></td>
				</tr>
				<tr ng-if="updateTypes[1].selected">
					<td>结题预期</td>
					<td>{{taskView.expectedEnd}}</td>
					<td><span ng-class="{'Emphasis':myForm.expectedEnd.$error.pattern || (commitAction && !updateView.expectedEnd)}">结题预期</span></td>
					<td><input type="text"   class="form-control" name="expectedEnd"  ng-pattern="/^20\d{2}$/" placeholder="请输入4位年份"  ng-model="updateView.expectedEnd" required/></td>
				</tr>
				<tr ng-if="updateTypes[3].selected">
					<td>研究内容</td>
					<td>{{taskView.projectContent}}</td>
					<td><span  ng-class="{'Emphasis': commitAction && !updateView.projectContent }">研究内容</span></td>
					<td><textArea rows="3" class="form-control" placeholder="简略说明主要研究内容" ng-model="updateView.projectContent" required></textArea></td>
				</tr>
				<tr ng-if="updateTypes[5].selected">
					<td>成果形式</td>
					<td>{{taskView.expectedGain}}</td>
					<td><span  ng-class="{'Emphasis': commitAction && !updateView.expectedGain }">成果形式</span></td>
					<td><textArea name="expectedGain" rows="3" class="form-control" placeholder="简略列举预期成果" ng-model="updateView.expectedGain" required></textArea></td>
				</tr>
				<tr ng-if="updateTypes[7].selected">
					<td></td>
					<td></td>
					<td><span  ng-class="{'Emphasis': commitAction && !updateView.memo }">其他变更</span></td>
					<td><input type="text" name="memo"  class="form-control" placeholder="请列出要变更的其他内容" ng-model="updateView.others" required/></td>
				</tr>
			</tbody>
</table>
</div>
