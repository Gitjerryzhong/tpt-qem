<div class="row">
<div ng-class="{'col-sm-9':updateView.groups=='未审'}">
...
<div class="panel panel-default form-body" >
<div class="panel-heading title">审批日志</div>
<div class="form-group" ng-if="auditList">
	<ul>
		<li ng-repeat="item in auditList"><span class="auditItem">{{dateFormat(item.date) | date:'yyyy-MM-dd'}}</span><span class="auditItem">{{item.userName}}</span><span class="auditItem">{{item.action | auditActionText}}</span><span class="auditItem">{{item.content}}</span></li>
	</ul>
</div>
</div>
</div>
<div class="col-sm-3" ng-if="updateView.groups=='未审'">
