...
<div class="form-group" style="border-bottom:0px;">
	 ...
	 <div class="col-sm-10" >
		<textarea  rows="6" class="form-control" placeholder="（变更项目负责人须写明新项目负责人的性别、出生时间、职称、工作单位、联系电话等情况；延期需要写明理由，延长日期及延期后的具体研究计划等。可附页说明。）" ng-model="updateForm.memo" required></textarea>
	 </div>
</div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-2">
    	<input type="text"  name="teacherName" class="form-control" placeholder="老师姓名" ng-model="updateForm.userName" ng-change="findTeacher(updateForm.userName)" required/>
    </div>
    <label for="currentTitle" class="col-sm-2 control-label" ng-class="{'Emphasis': commitAction && (!updateForm.teacherId) }" >工号</label>
    <div class="col-sm-2"  toolTip="正确输入姓名后才可以下拉选择">
    	<select class="form-control" 
    ng-model="updateForm.teacherId"
    ng-options="y.id as (y.id +'|'+y.name + '|'+ y.sex) for y in teachers" ></select>
    </div>
    <label for="currentDegree" class="col-sm-2 control-label" ng-class="{'Emphasis': commitAction && !updateForm.currentDegree}">学位</label>
    <div class="col-sm-2" >
    	  <select class="form-control" name="currentDegree"
    ng-model="updateForm.currentDegree"
    ng-options="y for y in degrees" >	
	</select>
    </div>
</div>
<div class="form-group">
    <label for="position" class="col-sm-2 control-label" ng-class="{'Emphasis': commitAction && !updateForm.position}">职务</label>
    <div class="col-sm-2">
    	 <select class="form-control" name="position"
	    	ng-model="updateForm.position"
	    	ng-options="y for y in positions"  >	
		</select>
    </div>
     <label for="currentTitle" class="col-sm-2 control-label" ng-class="{'Emphasis': commitAction && !updateForm.currentTitle}">职称</label>
    <div class="col-sm-2">
    	  <select class="form-control" name="currentTitle"
    ng-model="updateForm.currentTitle"
    ng-options="y for y in titles"  >	
	</select>
    </div>
    <label for="phoneNum" class="col-sm-2 control-label" ng-class="{'Emphasis':myForm.phoneNum.$error.pattern || (commitAction && !updateForm.phoneNum)}">电话</label>
    <div class="col-sm-2 " >
    	<input type="text" name="phoneNum"  class="form-control" placeholder="常用手机号码 " ng-model="updateForm.phoneNum" ng-pattern="/^((13[0-9])|(15[^4,\D])|(18[0,2,5-9]))\d{8}$/" required/>
    </div>
</div>
<div class="form-group">
	<label for="specailEmail" class="col-sm-2 control-label" ng-class="{'Emphasis': commitAction && !updateForm.specailEmail}">Email</label>
    <div class="col-sm-10 "  >
    	<input type="email"  name="specailEmail" class="form-control" placeholder="常用电子邮箱" ng-model="updateForm.specailEmail" required/>
    </div>
</div>
