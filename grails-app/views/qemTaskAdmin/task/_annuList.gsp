   <div class="form-group" ng-init="checkStatus='未安排评审'" style="margin-bottom:15px;">
		...
		<span class="pull-right"><button class="btn btn-primary " ng-if="checkStatus=='未安排评审'" ng-click="confirmAll(action.stage)">安排评审</button></span>
	</div>  
    <div class="input-group">
		...
		<select style="min-width:6em" class="form-control"    ng-model="trial.level"  ng-options="y.id as y.name for y in projectLevels"  ><option value="">全选</option></select>
...
<table class="table table-hover">
	<thead>
		<tr>
			...
			<th class="hand" style="width:5em" ng-click="orderBy('runStatus')">状态</th>	
		</tr>
	</thead>
	<tbody id="listBody">
		<tr ng-repeat="item in taskList | filter:{'groups':checkStatus} | filter: listConditions | orderBy:order" class='repeat-animation'>
