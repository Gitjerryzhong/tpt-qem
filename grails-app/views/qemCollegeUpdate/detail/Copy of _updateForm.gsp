...
<label class="col-sm-2 control-label">变更内容<span class="glyphicon glyphicon-star Emphasis"></span></label>
	<div class="col-sm-4" ng-init="status1.open=false;status2.open=false" ng-class="{'has-error': commitAction && !task.updateType}">
	...
	<div class="col-sm-4" ng-class="{'has-error': commitAction && task.updateType==6 && !task.origStyle}">
...
<div class="form-group">
    ...
    <div class="col-sm-2" ng-class="{'has-error': commitAction && (!task.userName || task.userName=='无此教师') }">
    	<input type="text"  ng-model="task.teacherId"  placeholder="请输入工号后回车校验" ng-keyup="myKeyup($event)" class="form-control"  toolTip="请输入工号后回车校验" required/>
    </div>
    <label for="currentDegree" class="col-sm-2 control-label">姓名</label>
    <div class="col-sm-2 form-control-static">
    	<span class="Emphasis"><strong>{{task.userName}}</strong></span>
    </div>
...
<div class="form-group">
    ...
    <div class="col-sm-4 " ng-class="{'has-error':commitAction && !task.members}">
...
<h4 class="col-md-offset-4 col-md-4 control-label" style="text-align:center"><strong>附    件</strong><a href="/tms/qemCollegeCheck/downloadAttch_T/{{task.id}}">（下载全部<span class="glyphicon glyphicon-download-alt"></span>）</a></h4>
</div>
</accordion-heading>
<div>
